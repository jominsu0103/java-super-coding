...
class AirReservationServiceUnitTest {
    ...
    @DisplayName("airlineTicket에 해당하는 user와 항공권들이 존재해 성공하는 경우")
    @Test
    void findUserFavoritePlaceTicketsCase1() {
        ...
        //when
        when(userRepository.findUserById(any())).thenReturn(Optional.of(userEntity));
        when(airlineTicketRepository.findAllAirlineTicketsWithPlaceAndTicketType(likePlace ,ticketType))
    ...
    @DisplayName("TicketType이 왕복 | 편도가 아닌 경우 , Exception이 발생해야함")
    @Test
    ...
    void findUserFavoritePlaceTicketsCase3() {
        ...
        //when
        when(userRepository.findUserById(any())).thenReturn(Optional.ofNullable(userEntity));
        when(airlineTicketRepository.findAllAirlineTicketsWithPlaceAndTicketType(likePlace ,ticketType))
                .thenReturn(airlineTickets);
        //then
